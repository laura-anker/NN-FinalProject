...
# Prepares any image for inference by cropping, resizing, converting to LAB, 
# and returning the L-channel tensor as well as the resized RGB image data
def prepare_image(image_path, target_size=224):
    ...
    # Extract the image dimensions and compute the size of the centered square crop
    h, w, _ = img.shape
    min_dim = min(h,w)
    top = (h - min_dim) // 2
    left = (w - min_dim) // 2
    ...
    # Resize the square image to the model's input resolution
    img_resized = cv2.resize(img_cropped, (target_size, target_size))
    
    # Convert the resized RGB image into LAB color space
    lab = cv2.cvtColor(img_resized, cv2.COLOR_RGB2LAB)

    # Normalize the L channel (0–255 → 0–1) and reshape it to (1, 1, H, W)
    L = lab[:, :, 0].astype("float32")
    L_norm = L / 255.0
    L_tensor = torch.tensor(L_norm).unsqueeze(0).unsqueeze(0).float()

    # Return the L channel tensor and the resized RGB image
